...
> The logo is in the public domain (see [Credits](#credits)). We are not affiliated with AdaCore.
...
[GitHub](https://github.com/bladeacer/Ada_CRDT). The
...
All serialised CRDT state begins with a protocol version byte. The version is
...
The demo is a real-time TUI simulation. It stress-tests eventual consistency
across three independent nodes. It uses `LWW_Element_Set` for cell state and
Yjs RGA for text rows.
...
Core packages (`CRDT.Core`, `CRDT.Pn_Counters`, `CRDT.Clocks.*`) are
