...
package body CRDT.Pn_Counters
  with SPARK_Mode => On
is

   use type CRDT.Core.Replica_Id;

   function Find_Actor (Entries : Actor_Array; Count : Natural; Actor : Core.Replica_Id) return Natural
   with Pre => Count <= Entries'Length and then Entries'First = 1, Post => (Find_Actor'Result = 0) or else (Find_Actor'Result in 1 .. Count and then Find_Actor'Result in Entries'Range)
   is
   begin
   ...
   procedure Increment (C : in out PN_Counter; By : Counter_Range := 1; Actor : Core.Replica_Id) is
      Idx    : Natural;
   ...
   procedure Decrement (C : in out PN_Counter; By : Counter_Range := 1; Actor : Core.Replica_Id) is
      Idx    : Natural;
      Old_Ct : constant Natural := C.Count;
   begin
      Idx := Find_Actor (C.Entries, C.Count, Actor);
   ...
   procedure Merge (Target : in out PN_Counter; Source : PN_Counter) is
      T_Idx  : Natural;
   ...
   begin
      for I in 1 .. Source.Count loop
         ...
         T_Idx := Find_Actor (Target.Entries, Target.Count, Source.Entries (I).Actor);
         if T_Idx = 0 then
   ...
   procedure Write_PN_Counter (Stream : not null access Ada.Streams.Root_Stream_Type'Class; Item : PN_Counter) with SPARK_Mode => Off is
   begin
   ...
   procedure Read_PN_Counter (Stream : not null access Ada.Streams.Root_Stream_Type'Class; Item : out PN_Counter) with SPARK_Mode => Off is
   ...
   begin
      ...
      if Count > Item.Max_Actors then
         raise Constraint_Error with "PN_Counter stream has more entries than Max_Actors";
      end if;
      Item.Count := Count;
      for I in 1 .. Count loop
         declare
         ...
         begin
            ...
            Item.Entries (I) := (Actor => Core.Replica_Id (Raw_A), P => Counter_Range (Raw_P), N => Counter_Range (Raw_N));
         end;
      end loop;
   end Read_PN_Counter;
