...
package body CRDT.Lww_Element_Sets
  with SPARK_Mode
is
   ...
   procedure Add (S : in out LWW_Element_Set; E : Element_Type; TS : Core.Lamport_Time) is
      Add_I    : Natural;
      Remove_I : Natural;
   begin
   ...
   procedure Remove (S : in out LWW_Element_Set; E : Element_Type; TS : Core.Lamport_Time) is
      Add_I    : Natural;
      Remove_I : Natural;
   begin
      Add_I := Find_Index (S.Add_Array, S.Add_Size, E);
   ...
   procedure Merge (Target : in out LWW_Element_Set; Source : LWW_Element_Set) is
   begin
   ...
   procedure Write_LWW_Element_Set (Stream : not null access Ada.Streams.Root_Stream_Type'Class; Item : LWW_Element_Set) with SPARK_Mode => Off is
   begin
   ...
   procedure Read_LWW_Element_Set (Stream : not null access Ada.Streams.Root_Stream_Type'Class; Item : out LWW_Element_Set) with SPARK_Mode => Off is
   ...
   begin
      ...
      if Add_Size > Item.Capacity or else Remove_Size > Item.Capacity then
         raise Constraint_Error with "LWW_Element_Set stream has more entries than Capacity";
      end if;
