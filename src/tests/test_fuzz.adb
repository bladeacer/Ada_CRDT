   ...
   procedure Run (RunR : in out Runner) is
      ...
      procedure Test_Fuzzing_Chaos is
         ...
         begin
            ...
            for Iter in 1 .. 20 loop
               declare
                  ...
                  Flip_Bits : constant Natural := Natural'Max (1, (Num_Bits * (1 + (Nat_Random.Random (Gen) mod 5))) / 100);
               ...
               begin
                  ...
                  for F_Iter in 1 .. Flip_Bits loop
                     ...
                     begin
                        Corrupt (Byte_Idx) := Corrupt (Byte_Idx) xor Stream_Element (2**Shift_Amt);
                     end;
                  end loop;
                  ...
                  exception
                     ...
                     when Ada.IO_Exceptions.Data_Error =>
                        null;
                     when E : others =>
                        RunR.Check (False, "Bit-flip fuzz: " & Ada.Exceptions.Exception_Name (E) & " at iter" & Natural'Image (Iter));
                  end;
               end;
            end loop;
         ...
         begin
            ...
            begin
               LWW.Merge (M, B);
               RunR.Check (True, "Clock skew: merge future + past sets completes without error");
               for I in 1 .. 10 loop
                  RunR.Check (LWW.Contains (M, I), "Clock skew: element" & Natural'Image (I) & " survives merge");
               end loop;
            end;
            ...
            begin
               LWW.Merge (M, A);
               RunR.Check (True, "Clock skew: merge past + future sets completes without error");
               for I in 1 .. 10 loop
                  RunR.Check (LWW.Contains (M, I), "Clock skew: element" & Natural'Image (I) & " survives reverse merge");
               end loop;
            end;
            ...
            begin
               ...
               LWW.Merge (A, C);
               RunR.Check (LWW.Contains (A, 42) and LWW.Contains (A, 99), "Clock skew: elements with mixed extreme timestamps present after merge");
            end;
         ...
         procedure Fuzz_Ooo_Delta_Flood is
            ...
            Cnt     : Natural;
            No_Seen : constant RGA_D.Replica_Max_Seq_Array (1 .. 2) := (1 => (Replica => 1, Max_Seq => 0), 2 => (Replica => 2, Max_Seq => 0));
         begin
            ...
            RunR.Check (RGA_D.Size (B) = 5, "Ooo flood: initial sync gives 5 elements (got" & Natural'Image (RGA_D.Size (B)) & ")");
            ...
            RunR.Check (RGA_D.Size (B) = 7, "Ooo flood: duplicate delta gives 7 elements (idempotent, got" & Natural'Image (RGA_D.Size (B)) & ")");
            ...
            begin
               ...
               RunR.Check (RGA_D.Size (B) = 7, "Ooo flood: incomplete SV does not duplicate (got" & Natural'Image (RGA_D.Size (B)) & ")");
            end;

            --  Stale SV (B already has everything, re-request)
            RGA_D.Sync_Delta (B, A, No_Seen, 2);
            RunR.Check (RGA_D.Size (B) = 7, "Ooo flood: stale SV does not duplicate (got" & Natural'Image (RGA_D.Size (B)) & ")");
      ...
      procedure Test_Fuzzing_Network_Partitions is
         ...
         procedure Do_Op (S : in out LWW.LWW_Element_Set; Stamp : in out Natural; Rep : CRDT.Core.Replica_Id; El : Integer) is
      ...
      begin
         ...
         begin
            ...
            RunR.Check (Divergent, "S3 diverged during partition (expected, confirms partition happened)");
         end;
         ...
         declare
            ...
            procedure Append_Char (R : in out RGA_Str.RGA; Seq : in out Natural; Rep : CRDT.Core.Replica_Id; Ch : Character) is
            begin
               Seq := Seq + 1;
         ...
         begin
            for I in 1 .. 50 loop
               Append_Char (R1, Seq_1, 1, Character'Val ((Nat_Random.Random (Gen) mod 26) + 65));
               Append_Char (R2, Seq_2, 2, Character'Val ((Nat_Random.Random (Gen) mod 26) + 65));
               Append_Char (R3, Seq_3, 3, Character'Val ((Nat_Random.Random (Gen) mod 26) + 65));
            end loop;
