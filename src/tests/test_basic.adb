...
package body Test_Basic is
   ...
   procedure Run (RunR : in out Runner) is
      ...
      procedure Test_LWW_Set is
         ...
         function Lamport (S : Natural; N : CRDT.Core.Replica_Id) return CRDT.Core.Lamport_Time
         is (Stamp => S, Node => N);
      begin
         New_Line;
         ...
         RunR.Check (not LWW.Contains (S, 42), "Remove (42, ts=150/1): Contains (42) = False (150 > 100)");

         LWW.Add (S, 42, Lamport (200, 1));
         RunR.Check (LWW.Contains (S, 42), "Re-add (42, ts=200/1): Contains (42) = True (200 > 150)");

         LWW.Remove (S, 42, Lamport (250, 1));
         RunR.Check (not LWW.Contains (S, 42), "Remove (42, ts=250/1): Contains (42) = False (250 > 200)");

         LWW.Remove (S, 7, Lamport (300, 2));
         RunR.Check (not LWW.Contains (S, 7), "Remove (7, ts=300/2): Contains (7) = False");

         LWW.Add (S, 7, Lamport (350, 1));
         RunR.Check (LWW.Contains (S, 7), "Re-add (7, ts=350/1): Contains (7) = True (350 > 300)");
         ...
         begin
            ...
            LWW.Merge (S, T);
            RunR.Check (LWW.Contains (S, 99), "Merge with set containing (99, ts=500/3): Contains (99) = True");
         end;
