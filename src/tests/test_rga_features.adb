...
package body Test_RGA_Features is
   ...
   procedure Run (RunR : in out Runner) is
      ...
      procedure Test_Chaotic_Interleaving is
         ...
         function El (C : Character) return Character
         is (C);
      ...
      begin
         ...
         begin
            ...
            RunR.Check (No_Exception, "Deleting unknown Node_Id does not raise an exception");
         exception
            when others =>
               RunR.Check (False, "Deleting unknown Node_Id raised an exception");
         end;

         RunR.Check (RGA_Str.Size (R) = 1, "Size unchanged after deleting unknown ID (got" & Natural'Image (RGA_Str.Size (R)) & ")");

         RGA_Str.Delete_Node (R, Known_Id);
         RunR.Check (RGA_Str.Size (R) = 1, "Size unchanged after deleting known ID (tombstone)");

         declare
            No_Exception : Boolean := True;
         begin
            RGA_Str.Delete_Node (R, Known_Id);
            RunR.Check (No_Exception, "Deleting already-deleted ID does not raise an exception");
         exception
            when others =>
      ...
      begin
         ...
         RunR.Check (RGA_Str.Size (B) >= 4, "After delta sync B size >= 4 (got" & Natural'Image (RGA_Str.Size (B)) & ")");
      ...
      begin
         ...
         begin
            RGA_Str.Sync_Delta (B, A, SV, SV_Cnt);
            RunR.Check (No_Exception, "Delta with unseen replica does not raise an exception");
         exception
            when others =>
               RunR.Check (False, "Delta with unseen replica raised an exception");
         end;

         RunR.Check (RGA_Str.Size (B) = 3, "After ooo delta B has all 3 elements (got" & Natural'Image (RGA_Str.Size (B)) & ")");
         ...
         begin
            ...
            begin
               RGA_Str.Sync_Delta (B, A, SV, SV_Cnt);
               RunR.Check (No_Exception, "Delta with incomplete SV does not raise an exception");
            exception
               when others =>
                  RunR.Check (False, "Delta with incomplete SV raised an exception");
            end;
            RunR.Check (RGA_Str.Size (B) = 3, "After incomplete SV delta B still has 3 elements (got" & Natural'Image (RGA_Str.Size (B)) & ")");
         end;
