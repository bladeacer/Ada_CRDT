...
package body Test_Serialization is
   ...
   procedure Run (RunR : in out Runner) is
      ...
      begin
         ...
         RunR.Check (RGA_Str.Size (Dst) = 6, "Deserialized size = 6 (got" & Natural'Image (RGA_Str.Size (Dst)) & ")");
      ...
      begin
         ...
         RGA_Str.RGA'Read (Stream (F), Dst);
         Close (F);
         RunR.Check (RGA_Str.Size (Dst) = 0, "Empty RGA round-trip: size = 0");
         declare
            use type RGA_Str.RGA;
         begin
         ...
         RGA_Str.RGA'Read (Stream (F), Dst);
         Close (F);
         RunR.Check (RGA_Str.Get (Dst, 1) = Character'Val (0), "Null byte round-trip: Get(1) = 0");
         declare
            use type RGA_Str.RGA;
         begin
         ...
         RGA_Str.RGA'Read (Stream (F), Dst);
         Close (F);
         RunR.Check (RGA_Str.Get (Dst, 1) = Character'Val (0), "High byte round-trip: Get(1) = 0 (unchanged)");
         RunR.Check (RGA_Str.Get (Dst, 2) = Character'Val (255), "High byte round-trip: Get(2) = 255");
         declare
            use type RGA_Str.RGA;
         begin
      ...
      begin
         ...
         RunR.Check (RGA_Str.Size (Dst) = 5, "V1 migration: deserialized size = 5 (got" & Natural'Image (RGA_Str.Size (Dst)) & ")");
         ...
         begin
            WNE (2);
            WNE (0);
            WNE (0);  -- Version=2, Total=0, Count=0
            ...
            Close (F);
            RunR.Check (RGA_Str.Size (Empty_Dst) = 0, "V1 migration: empty RGA deserialized size = 0");
         end;
      ...
      begin
         ...
         WN (1);
         WN (5);
         WN (2);   -- Actor=1, P=5, N=2
         WN (2);
         WN (3);
         WN (0);   -- Actor=2, P=3, N=0
         ...
         begin
            ...
            RunR.Check (CRDT.Pn_Counters.Value (C) = 6, "PN V1 migration: value = 5+3-2 = 6 (got" & Integer'Image (CRDT.Pn_Counters.Value (C)) & ")");
            ...
            RunR.Check (CRDT.Pn_Counters.Value (C) = CRDT.Pn_Counters.Value (D), "PN V1->V2 round-trip: values match");
         end;
         ...
         begin
            ...
            CRDT.Pn_Counters.PN_Counter'Read (Stream (F), C);
            Close (F);
            RunR.Check (CRDT.Pn_Counters.Value (C) = 0, "PN V1 migration: empty counter value = 0");
            RunR.Check (CRDT.Pn_Counters.Value (C) = CRDT.Pn_Counters.Value (D), "PN V1 migration: empty counter matches fresh");
         end;
      ...
      begin
         ...
         Migrate_Header (Source => Stream (F_In), Dest => Stream (F_Out), Kind => Kind, Total => Total, Count => Count);

         RunR.Check (Kind = Proto_V1, "Migrate_Header: detected V1");
         RunR.Check (Total = 42, "Migrate_Header: Total = 42, got" & Natural'Image (Total));
         RunR.Check (Count = 7, "Migrate_Header: Count = 7, got" & Natural'Image (Count));

         Close (F_In);
         Close (F_Out);

         Open (F_Out, In_File, "/tmp/crdt_v2_migrate_out.bin");
         Read_Header (Stream => Stream (F_Out), Kind => Kind, Total => V2_Total, Count => V2_Count, Clock_Kind => Ignore_CK);
         RunR.Check (Kind = Proto_V2, "Migrate_Header: output is V2");
         RunR.Check (V2_Total = 42, "Migrate_Header: V2 Total = 42, got" & Natural'Image (V2_Total));
         RunR.Check (V2_Count = 7, "Migrate_Header: V2 Count = 7, got" & Natural'Image (V2_Count));
         Close (F_Out);
         ...
         RunR.Check (Kind = Proto_V2, "Migrate_Header: detected V2 input");
         RunR.Check (Total = 10, "Migrate_Header (V2): Total = 10, got" & Natural'Image (Total));
         RunR.Check (Count = 3, "Migrate_Header (V2): Count = 3, got" & Natural'Image (Count));
         ...
         Read_Header (Stream => Stream (F_Out), Kind => Kind, Total => V2_Total, Count => V2_Count, Clock_Kind => Ignore_CK);
         RunR.Check (Kind = Proto_V2, "Migrate_Header: V2->V2 output is V2");
         RunR.Check (V2_Total = 10, "Migrate_Header: V2->V2 Total = 10, got" & Natural'Image (V2_Total));
         RunR.Check (V2_Count = 3, "Migrate_Header: V2->V2 Count = 3, got" & Natural'Image (V2_Count));
         Close (F_Out);
      ...
      begin
         ...
         RunR.Check (Kind = Proto_V1, "V1 back-compat: empty payload detected as V1");
         RunR.Check (Total = 0, "V1 back-compat: empty Total = 0, got" & Natural'Image (Total));
         RunR.Check (Count = 0, "V1 back-compat: empty Count = 0, got" & Natural'Image (Count));
         Close (F);
         ...
         Read_Header (Stream (F), Kind, Total, Count, Ignore_CK);
         RunR.Check (Kind = Proto_V1, "V1 back-compat: max payload detected as V1");
         RunR.Check (Total = Natural'Last, "V1 back-compat: max Total, got" & Natural'Image (Total));
         RunR.Check (Count = Natural'Last, "V1 back-compat: max Count, got" & Natural'Image (Count));
         Close (F);
         ...
         WB (2);
         WB (0);
         WB (0);  -- V2: Version=2, Total=0, Count=0
         Create (F, Out_File, "/tmp/crdt_v2_proto_check.bin");
         Ada.Streams.Stream_IO.Write (F, Buf (1 .. Idx - 1));
         Close (F);
         Open (F, In_File, "/tmp/crdt_v2_proto_check.bin");
         Read_Header (Stream (F), Kind, Total, Count, Ignore_CK);
         RunR.Check (Kind = Proto_V2, "V1 back-compat: V2 short header detected as V2");
         Close (F);
         ...
         begin
            ...
            RGA_Str.RGA'Read (Stream (F), Dst);
            Close (F);
            RunR.Check (RGA_Str.Size (Dst) = 3, "V1 back-compat: multi-item RGA size = 3, got" & Natural'Image (RGA_Str.Size (Dst)));
            RunR.Check (RGA_Str.Get (Dst, 1) = 'A', "V1 back-compat: multi-item RGA Get(1)='A'");
            RunR.Check (RGA_Str.Get (Dst, 3) = 'C', "V1 back-compat: multi-item RGA Get(3)='C'");
         end;
         ...
         begin
            ...
            WN (2);
            WN (1);
            WN (1);  -- Ver=2, Add_Size=1, Remove_Size=1
            ...
            Close (F);
            RunR.Check (not LWW.Contains (S, 42), "V1 back-compat: LWW remove, not contains 42");
         end;
      ...
      procedure Test_LEB128 is
         use Ada.Streams;
         use Ada.Streams.Stream_IO;
         Values : constant array (Positive range <>) of Natural := (0, 1, 127, 128, 16383, 16384, 2097151, 2097152, Natural'Last);
