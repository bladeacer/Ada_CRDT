   ...
   procedure Run (RunR : in out Runner) is
      ...
      begin
         ...
         begin
            RGA_Str.Merge (M, R2);
            RGA_Str.Merge (M, R3);
            RunR.Check (RGA_Str.Size (M) >= 5, "Unified state size >= 5 (got" & Natural'Image (RGA_Str.Size (M)) & ") -- " & To_String (M));
         end;
         ...
         begin
            RGA_Str.Merge (M12, R2);
            RGA_Str.Merge (M12, R3);
            RGA_Str.Merge (M21, R1);
            RGA_Str.Merge (M21, R3);
            RGA_Str.Merge (M13, R3);
            RGA_Str.Merge (M13, R2);
            RGA_Str.Merge (M31, R1);
            RGA_Str.Merge (M31, R2);
            RGA_Str.Merge (M23, R3);
            RGA_Str.Merge (M23, R1);
            RGA_Str.Merge (M32, R2);
            RGA_Str.Merge (M32, R1);

            declare
               Siz : constant Natural := RGA_Str.Size (M12);
            begin
               RunR.Check
                 (RGA_Str.Size (M21) = Siz and then RGA_Str.Size (M13) = Siz and then RGA_Str.Size (M31) = Siz and then RGA_Str.Size (M23) = Siz and then RGA_Str.Size (M32) = Siz,
                  "All 6 merge orders have same size =" & Natural'Image (Siz));
            end;
            ...
            begin
               ...
               RunR.Check (RGA_Str.Size (Tmp) = RGA_Str.Size (M12), "Merge(M12, M21) preserves size");
            end;
            declare
               Tmp : RGA_Str.RGA (Max_RGA) := M12;
            begin
               RGA_Str.Merge (Tmp, M31);
               RunR.Check (RGA_Str.Size (Tmp) = RGA_Str.Size (M12), "Merge(M12, M31) preserves size");
            end;
         end;
         ...
         begin
            ...
            begin
               ...
               begin
                  ...
                  begin
                     RunR.Check (RGA_Str.Size (M1) >= 12, "Anti-interleaving: M1 size >= 12 (got" & Natural'Image (RGA_Str.Size (M1)) & ")");
                     RunR.Check (RGA_Str.Size (M2) >= 12, "Anti-interleaving: M2 size >= 12 (got" & Natural'Image (RGA_Str.Size (M2)) & ")");

                     RunR.Check
                       (Has_Substr (S1, "H") and Has_Substr (S1, "e") and Has_Substr (S1, "l") and Has_Substr (S1, "o") and Has_Substr (S1, " "),
                        "Anti-interleaving: M1 has all base chars (got """ & S1 & """)");
                     RunR.Check
                       (Has_Substr (S2, "H") and Has_Substr (S2, "e") and Has_Substr (S2, "l") and Has_Substr (S2, "o") and Has_Substr (S2, " "),
                        "Anti-interleaving: M2 has all base chars (got """ & S2 & """)");

                     RunR.Check
                       (Has_Substr (S1, "1") and Has_Substr (S1, "2") and Has_Substr (S1, "3") and Has_Substr (S1, "A") and Has_Substr (S1, "B") and Has_Substr (S1, "C"),
                        "Anti-interleaving: M1 has all 6 inserted chars (got """ & S1 & """)");
                     RunR.Check
                       (Has_Substr (S2, "1") and Has_Substr (S2, "2") and Has_Substr (S2, "3") and Has_Substr (S2, "A") and Has_Substr (S2, "B") and Has_Substr (S2, "C"),
      ...
      begin
         ...
         RunR.Check (LWW.Contains (A, 999), "Clock skew: local add of element 999 with Stamp=1 is present");
      ...
      begin
         ...
         begin
            for I in 1 .. 90 loop
               RGA_Ch.Insert (RG, RGA_Ch.Size (RG) + 1, (1, I), Character'Val ((I - 1) mod 26 + 65));
            end loop;
         ...
         RunR.Check (RGA_Ch.Count (RG) >= 80, "Tombstone saturation: Count >= 80 (got" & Natural'Image (RGA_Ch.Count (RG)) & ")");
         RunR.Check (RGA_Ch.Size (RG) >= 10, "Tombstone saturation: Size >= 10 (got" & Natural'Image (RGA_Ch.Size (RG)) & ")");

         RGA_Ch.Compact (RG);

         RunR.Check (RGA_Ch.Size (RG) >= 10, "Tombstone saturation: after compact Size >= 10 (got" & Natural'Image (RGA_Ch.Size (RG)) & ")");
         RunR.Check (RGA_Ch.Count (RG) >= 10, "Tombstone saturation: after compact Count >= 10 (got" & Natural'Image (RGA_Ch.Count (RG)) & ")");
         ...
         RunR.Check (RGA_Ch.Size (RG) >= 40, "Tombstone saturation: final Size >= 40 (got" & Natural'Image (RGA_Ch.Size (RG)) & ")");
