...
package body Test_Engines is
   ...
   procedure Run (RunR : in out Runner) is
      ...
      begin
         ...
         declare
            Config : CRDT.Sync.State_Based.Sync_Config := (Max_Replicas => 4, Delta_Sync => True, HLC_Node => 1);
            Local  : CRDT.Sync.State_Based.Replica_State := CRDT.Sync.State_Based.Create (Config);
            Remote : CRDT.Sync.State_Based.Replica_State := CRDT.Sync.State_Based.Create (Config);
         begin
         ...
         begin
            ...
            CRDT.Sync.Op_Based.Append (Log, (Kind => CRDT.Sync.Op_Based.Op_Delete, Seq => 2, Node => 1, Del_Position => 1));
            RunR.Check (CRDT.Sync.Op_Based.Size (Log) = 2, "Op-based sync: log size = 2");

            CRDT.Sync.Op_Based.Acknowledge (Log, 1);
            RunR.Check (CRDT.Sync.Op_Based.Size (Log) = 1, "Op-based sync: after ack up to seq 1, size = 1");

            CRDT.Sync.Op_Based.Compact (Log);
            RunR.Check (CRDT.Sync.Op_Based.Size (Log) = 1, "Op-based sync: after compact, size = 1");

            declare
               Op : constant CRDT.Sync.Op_Based.Operation := CRDT.Sync.Op_Based.Get (Log, 1);
            begin
               RunR.Check (Op.Kind = CRDT.Sync.Op_Based.Op_Delete, "Op-based sync: Get remaining op = Delete");
            end;
         end;
