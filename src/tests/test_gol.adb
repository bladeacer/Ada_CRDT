   ...
   procedure Run (RunR : in out Runner) is
      ...
      procedure Test_GoL_Neighbors is
         ...
         begin
            for DR in -1 .. 1 loop
               for DC in -1 .. 1 loop
                  if (DR /= 0 or DC /= 0) and then Cell_Set.Contains (S, (Row + DR) * 100 + Col + DC) then
                     C := C + 1;
                  end if;
               end loop;
            end loop;
      ...
      begin
         ...
         RunR.Check (Count_Neighbors (S, 5, 6) = 2, "Blinker center has 2 neighbors (got" & Natural'Image (Count_Neighbors (S, 5, 6)) & ")");
         RunR.Check (Count_Neighbors (S, 4, 6) = 3, "Above blinker center has 3 neighbors (got" & Natural'Image (Count_Neighbors (S, 4, 6)) & ")");
         RunR.Check (Count_Neighbors (S, 6, 6) = 3, "Below blinker center has 3 neighbors (got" & Natural'Image (Count_Neighbors (S, 6, 6)) & ")");
         RunR.Check (Count_Neighbors (S, 1, 1) = 0, "Corner cell has 0 neighbors (got" & Natural'Image (Count_Neighbors (S, 1, 1)) & ")");
      ...
      procedure Test_GoL_Blinker is
         ...
         function Is_Alive (S : Cell_Set.LWW_Element_Set; R, C : Integer) return Boolean
         is (Cell_Set.Contains (S, R * 100 + C));
         ...
         begin
            for DR in -1 .. 1 loop
               for DC in -1 .. 1 loop
                  if (DR /= 0 or DC /= 0) and then Is_Alive (S, Row + DR, Col + DC) then
                     C := C + 1;
                  end if;
               end loop;
            end loop;
         ...
         function Is_Blinker (S : Cell_Set.LWW_Element_Set; R, C : Integer) return Boolean
         is ((R = 5 and C in 5 .. 7) or (R = 6 and C = 6));
      begin
         New_Line;
         ...
         begin
            ...
            Stamp := 1;
            Cell_Set.Add (Cells, 5 * 100 + 5, (Stamp => 1, Node => 1));
            Stamp := 2;
            Cell_Set.Add (Cells, 5 * 100 + 6, (Stamp => 2, Node => 1));
            Stamp := 3;
            Cell_Set.Add (Cells, 5 * 100 + 7, (Stamp => 3, Node => 1));

            RunR.Check (Is_Alive (Cells, 5, 5) and Is_Alive (Cells, 5, 6) and Is_Alive (Cells, 5, 7), "Blinker gen 0: horizontal alive");

            --  Evolve to generation 1 (vertical)
            Evolve;
            RunR.Check (not Is_Alive (Cells, 5, 5) and Is_Alive (Cells, 4, 6) and Is_Alive (Cells, 5, 6) and Is_Alive (Cells, 6, 6), "Blinker gen 1: vertical alive");

            --  Evolve to generation 2 (horizontal again -- oscillator period 2)
            Evolve;
            RunR.Check (Is_Alive (Cells, 5, 5) and Is_Alive (Cells, 5, 6) and Is_Alive (Cells, 5, 7), "Blinker gen 2: back to horizontal");
         end;
      ...
      begin
         ...
         for R in 1 .. Grid_Size loop
            ...
            for C in 1 .. Grid_Size loop
               Seq := Seq + 1;
               Rows.Insert (Grid (R), C, (Id, Seq), (if Matrix.Contains (Cells, R * 100 + C) then '#' else '.'));
            end loop;
         end loop;
         ...
         begin
            ...
            begin
               for R in 1 .. Grid_Size loop
                  for C in 1 .. Grid_Size loop
                     if Matrix.Contains (Cells, R * 100 + C) /= (Rows.Get (Grid (R), C) = '#') then
                        Match := False;
                     end if;
                  end loop;
               end loop;
            ...
            RunR.Check (Seq > Old_Seq, "Matrix -> Yjs: seq counter monotonically increases (" & Natural'Image (Seq) & " >" & Natural'Image (Old_Seq) & ")");
         end;
      ...
      procedure Test_GoL_Convergence is
         ...
         begin
            for DR in -1 .. 1 loop
               for DC in -1 .. 1 loop
                  if (DR /= 0 or DC /= 0) and then Row + DR in 1 .. Grid_Size and then Col + DC in 1 .. Grid_Size and then Is_Alive (S, Row + DR, Col + DC) then
                     C := C + 1;
                  end if;
               end loop;
            end loop;
      ...
      begin
         ...
         Cell_Set.Merge (N1, N2);
         Cell_Set.Merge (N2, N1);
         Cell_Set.Merge (N1, N3);
         Cell_Set.Merge (N3, N1);
         Cell_Set.Merge (N2, N3);
         Cell_Set.Merge (N3, N2);
      ...
      begin
         ...
         begin
            RunR.Check (Seq_After_First = Grid_Size * Grid_Size, "Mode switch gen 0: seq = Grid_Size^2 (" & Natural'Image (Seq_After_First) & " = " & Natural'Image (Grid_Size * Grid_Size) & ")");
         end;
         ...
         RunR.Check (Seq = Grid_Size * Grid_Size * 2, "Mode switch gen 1: seq monotonically increased (" & Natural'Image (Seq) & ")");
