...
"576 total checks" row in the ledger, past-release changelogs) are left
...
--run-prove  Run prove via adacovex first to refresh obj/gnatprove/gnatprove.out
...
def parse_summary(path: Path) -> Metrics:
    ...
    # Same rules as Determine_SPARK_Level in adacovex.
...
def replacements(m: Metrics) -> List[Tuple[Pattern[str], str]]:
    ...
    return [
        # "Platinum (589 VCs)" -- level + total together.
        (re.compile(rf"{LEVEL_RE} \((\d+) VCs\)"), f"{lvl} ({t} VCs)"),
        # "Platinum, 589 VCs" -- comma form used in Makefile help text.
        (re.compile(rf"{LEVEL_RE}, (\d+) VCs"), f"{lvl}, {t} VCs"),
        # "Platinum (589 VCs, 0 unproved under gnatprove 16.1.0)".
        ...
        # "--require-spark=Platinum" and "require-spark: Platinum".
...
def main() -> int:
    ...
    ap.add_argument("--run-prove", action="store_true",
                    help="refresh obj/gnatprove/gnatprove.out via make prove")
    ...
    if args.run_prove:
        # Use adacovex prove via Makefile -- triggers sibling or alr exec path
        subprocess.run(["make", "prove"], check=True, cwd=str(ROOT))
    ...
    if args.dry_run:
        ...
        for pat, rep in replacements(m):
            print(f"  pattern: {pat.pattern!r} -> {rep!r}")
        return 0
    ...
    if matched == 0:
        print("warning: no proof-metric pattern matched; docs may not contain anchored phrases (ok if intentional)",
              file=sys.stderr)
        # Not an error for CRDT -- many docs don't carry the anchored pattern yet
        print(f"scanned {len(files)} live file(s), no anchored phrases found")
        return 0
