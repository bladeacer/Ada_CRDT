#!/usr/bin/env python3
"""Sync the crdt crate description from a single source of truth.
...
alire.toml, alire-dev.toml, every release manifest (alire/releases/crdt-*.toml),
and every local index entry (index/ad/crdt/crdt-*.toml).  They drifted by hand
in the past, so this script keeps them all identical to the canonical files:
...
MANIFESTS: List[Path] = [
    ROOT / "alire.toml",
    ROOT / "alire-dev.toml",
    *sorted((ROOT / "alire" / "releases").glob("crdt-*.toml")),
    *sorted((ROOT / "index" / "ad" / "crdt").glob("crdt-*.toml")),
]
