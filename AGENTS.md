...
vendored [SimpleEnglish skill](skills/simple-english/) (`SKILL.md` plus
`references/word-swaps.md`, `references/use-cases.md`, `references/checklist.md`)
to apply ASD-STE100 rules. Load it from that path.

The controlled list of Technical Names (non-STE words approved for the CRDT
domain, with part of speech, definition, and banned synonyms) lives in
[docs/ste100-technical-names.md](docs/ste100-technical-names.md). Add any new
Technical Name to that list before you use it in documentation, docstrings, or
changelogs.

The skill enforces short sentences, active voice, one instruction per sentence,
consistent terminology, and no AI-slop hedging. This project sets two overrides
on top of the skill's defaults:

- Use British English spelling, not the American spelling that Rule 1.14 of the
  skill names.
- Keep each paragraph to a maximum of four sentences, not the six that Rule 6.6
  of the skill names.
...
The vendored `skills/` directory (third-party upstream text from the
SimpleEnglish skill) is excluded from the scan, the same as in the sibling
adacovex project.
